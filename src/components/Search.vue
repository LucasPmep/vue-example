<template>
    <div class="container text-center mr-auto ml-auto">
        <input type="text" v-model="q" @change="search(q)" placeholder="Search person or company name" class="mb-8 text-black w-80 p-2">
        <div v-if="searching">
            <div v-if="loading">
                <Loading />
            </div>
            <div v-else  class="flex flex-row text-center mr-auto ml-auto w-full">
                <div class="m-4 w-full">
                    <h2>Persons :</h2>
                    <div v-if="persons[0] == null">
                        <div class="bg-gray-500 text-black mb-4 p-1 border-blue-100 border-2 m-2">
                            Aucun résultat trouvé
                        </div>
                    </div>
                    <div v-for="person in persons" :key="person.id" class="m-2">
                        <div class="bg-black text-white mb-4 p-1 border-blue-300 border-2">
                            <router-link :to="{ name: 'person.detail', params: { id: person.id } }">
                                <ul>
                                    <li>{{ person.firstname }} {{ person.lastname }}</li>
                                </ul>
                            </router-link>
                        </div>
                    </div>
                </div>
                <div class="m-4 w-full">
                    <h2>Companies : </h2>
                    <div v-if="companies[0] == null">
                        <div class="bg-gray-500 text-black mb-4 p-1 border-blue-100 border-2 m-2">
                            Aucun résultat trouvé
                        </div>
                    </div>
                    <div v-for="company in companies" :key="company.id" class="m-2">
                        <div class="bg-black text-white mb-4 p-1 border-red-300 border-2">
                            <router-link :to="{ name: 'company.detail', params: { id: company.id } }">
                                <ul>
                                    <li>{{ company.name }}</li>
                                </ul>
                            </router-link>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div v-if="searching == false" class="text-gray-300 text-md">
            Try to search any name you want !
        </div>
    </div>
</template>

<script setup>
import { ref } from "vue";
import usePersons from "../services/personservices.js";
import useCompanies from "../services/companyservices.js";
import Loading from "./Loader.vue";

const { persons, searchPerson } = usePersons();
const { companies, searchCompany } = useCompanies();
const q = ref('');
const searching = ref(false);
const loading = ref(false);
await searchPerson(q);
await searchCompany(q);


const search = async (q) => {
    if (q == '') {
        searching.value = false
    } else {
        loading.value = true;
        searching.value = true
        await searchPerson(q);
        await searchCompany(q);
        loading.value = false;
    }

}

</script>